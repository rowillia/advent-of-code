...
class PageOrderingRule:
    ...
    @classmethod
    def parse(cls, text: str) -> "PageOrderingRule":
        return cls(*map(int, text.strip().split("|")))


@dataclass()
class PageOrdering:
    rules: list[PageOrderingRule]
    before_index: dict[int, set[int]]
    after_index: dict[int, set[int]]

    def is_valid(self, update: list[int]) -> bool:
        seen: set[int] = set()
        for page in update:
            if self.before_index[page].intersection(seen):
                return False
            seen.add(page)

        return True

    def correct(self, update: list[int]) -> list[int]:
        page_set: set[int] = set(update)
        dep_grap = {
            page: page_set.intersection(self.before_index[page]) for page in update
        }
        sorter = TopologicalSorter(dep_grap)
        return list(sorter.static_order())

    @classmethod
    def parse(cls, text: str) -> "PageOrdering":
        rules = list(map(PageOrderingRule.parse, text.splitlines()))
        before_index: dict[int, set[int]] = defaultdict(set)
        after_index: dict[int, set[int]] = defaultdict(set)
        for rule in rules:
            after_index[rule.after].add(rule.before)
            before_index[rule.before].add(rule.after)

        return cls(rules, before_index, after_index)
...
class Updates:
    ...
    @classmethod
    def parse(cls, text: str) -> "Updates":
        return cls([list(map(int, line.split(","))) for line in text.splitlines()])
...
class Problem:
    ...
    def valid_updates(self):
        return [
            update for update in self.updates.pages if self.ordering.is_valid(update)
        ]

    def corrected_updates(self):
        invalid_updates = [
            update
            for update in self.updates.pages
            if not self.ordering.is_valid(update)
        ]
        return [self.ordering.correct(update) for update in invalid_updates]

    @classmethod
    def parse(cls, text: str) -> "Problem":
        ordering, updates = text.split("\n\n")
        return cls(PageOrdering.parse(ordering), Updates.parse(updates))


def part1(text: str) -> int | None:
    problem = Problem.parse(text)
    return sum(update[len(update) // 2] for update in problem.valid_updates())


def part2(text: str) -> int | None:
    problem = Problem.parse(text)
    return sum(update[len(update) // 2] for update in problem.corrected_updates())
