...
def count_in_steps(value: int, count: int):
    if count == 0:
        ...
        return count_in_steps(1, count - 1)
    elif (str_len := len(value_as_str := str(value))) % 2 == 0:
        return count_in_steps(
            int(value_as_str[: str_len // 2]), count - 1
        ) + count_in_steps(int(value_as_str[str_len // 2 :]), count - 1)
    else:
        return count_in_steps(value * 2024, count - 1)


def part1(text: str) -> int | None:
    return sum(count_in_steps(int(x), 25) for x in text.split())


def part2(text: str) -> int | None:
    return sum(count_in_steps(int(x), 75) for x in text.split())
