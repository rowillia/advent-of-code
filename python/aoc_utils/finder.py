...
def get_days(years: List[int] | None = None) -> List[Day]:
    ...
    if years is None:
        years = [int(d.name) for d in (project_path / "solutions").iterdir() if d.is_dir()]
    ...
    for year in years:
        ...
        for py_file in solutions_path.glob("*.py"):
            ...
            days.append(
                Day(year, int(day_number), module, solution_input, str(example_yaml['input']), [str(x) for x in example_yaml['answers']])
            )
