...
from aoc.common.graph import astar
from aoc.common.point import Point


@dataclass
class Memory:
    corrupted: frozenset[Point]
    lower_right_bound: Point

    @classmethod
    def parse(cls, text: str, limit: int | None = None):
        corrupted = set()
        for line in text.splitlines():
            corrupted.add(Point(*[int(x) for x in line.split(",")]))
            if limit is not None and len(corrupted) >= limit:
                break
        return cls(
            frozenset(corrupted),
            Point(max(p.x for p in corrupted) + 1, max(p.y for p in corrupted) + 1),
        )

    def sovlve(self):
        def heuristic(p1: Point, p2: Point):
            return p1.manhattan_distance(p2)

        def neighbors(p1: Point):
            for point in p1.adjacent_points(
                diagonal=False, upper_bound=self.lower_right_bound
            ):
                if point not in self.corrupted:
                    yield point, 1
        ...
        return (
            len(
                astar(
                    Point(0, 0),
                    Point(self.lower_right_bound.x - 1, self.lower_right_bound.y - 1),
                    heuristic,
                    neighbors,
                )
            )
...
def part1(text: str) -> int | None:
    limit = 1024 if len(text.splitlines()) > 1024 else 12
    return Memory.parse(text, limit).sovlve()


def part2(text: str) -> str | None:
    line_count = len(text.splitlines())
    low, hi = 0, line_count - 1
    while (hi - low) > 1:
        guess = ((hi - low) // 2) + low
        try:
            Memory.parse(text, guess).sovlve()
            low = guess
        except Exception:
            hi = guess
    return text.splitlines()[low]
