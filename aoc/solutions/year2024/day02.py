...
def is_safe(level: list[int]) -> bool:
    for i, x in enumerate(level[1:], 1):
        diff = x - level[i - 1]
        if not 0 < abs(diff) < 4:
            return False
        if i > 1 and (diff > 0) != ((level[i - 1] - level[i - 2]) > 0):
            return False
    return True
...
def part1(text: str) -> int | None:
    return sum(
        int(is_safe([int(x) for x in line.split()])) for line in text.splitlines()
    )
    ...
    for line in text.splitlines():
        ...
        if is_safe(parsed_line):
            ...
            for i in range(len(parsed_line)):
                if is_safe(parsed_line[:i] + parsed_line[i + 1 :]):
                    total += 1
                    break
    return total
